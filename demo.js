...
function searchInDataForCategory(catg) {
  if (categoryjson[catg] !== undefined) {
    console.log("Competitors:\n");
    for (var i = 1; i <= categoryjson[catg].length; i++) {
      console.log("" + i + " - " + categoryjson[catg][i]);
    }
  } else {
...
async function gcse_run(options, catg) {
  ...
  if (res.data.items !== undefined) {
    ...
    for (var i = 0; i < res.data.items.length; i++) {
      ...
      if (proto + hostname !== sites1[0] && k < 5) {
        ...
        categoryjson[catg].push(hostname);
...
const sendForScraping = async function(catg, sites, keywords_str_arr = []) {
  var keywords_arr = [];
  var siteurl = sites;
  var payload = {
    category: catg,
      ...
      if (!error) {
        ...
        if (len > 10) {
          for (var j = 0; j < num; j++) {
            ...
            for (var z = 0; z < 10; z++) {
              if (newjson[catg] === undefined) {
                newjson[catg] = [];
              }
              newjson[catg].push(keywords_arr[j][z]);
            }
...
async function foo(catg) {
  sites2 = output;
  for (var z = 0; z < output.length; z++) {
    keywords_str_arr = [];
    // make sites2[z] an array
    sendForScraping(catg, [sites2[z]], keywords_str_arr);
  }
  var t1 = performance.now();
  console.log("It took " + (t1 - t0) / 1000 + " seconds.");
}
...
router.get("/", (req, res) => {
  ...
  if (req.query.site === undefined) {
    ...
    searchInDataForCategory(req.query.category);
    // "await" must be used here before sendForScraping
